...
class LinkTypeFilter(BaseModel):
    allow_patterns: Optional[List[str]] = Field(
        default=None, description="Lista regexów dopuszczających linki"
    )
    deny_patterns: Optional[List[str]] = Field(
        default=None, description="Lista regexów blokujących linki"
    )
...
class CrawlerConfig(BaseModel):
    ...
    obey_rate_limit: Optional[float] = Field(
        default=None, description="Sekundy pauzy pomiędzy żądaniami"
    )
    max_concurrency: int = Field(10, ge=1, le=50)
    export_store_body: bool = Field(
        True, description="Czy zapisywać całe body (ucięte do limitu)"
    )
    max_body_chars: int = Field(200_000, ge=1000, le=2_000_000)
    respect_content_type_html_only: bool = Field(
        True, description="Jeśli True, pobieraj tylko HTML"
    )
